...
def start():
    ...
    hyperparameters_default = dict(
        SEED = 123,
        TRAIN_FILE = './datasets/n_500_p_0_1/dataset_2.csv',
        LEARNING_RATE = 0.001,
        NUM_EPOCHS = 25,
        IMAGE_SIZE = 224,
        NUM_CLASSES = 2,
    ...
    run = wandb.init(project="imbalance_mitigation_chest_x_ray", config=hyperparameters_default)
    config = wandb.config

    file_name = config.TRAIN_FILE.split('/')

    run.name = 'down_'+file_name[-2] + '_' + file_name[-1].replace('.csv', '')
    ...
    aug_func = horizontal_flip

    # Images Loaders
    train_loader = get_dataloaders(config.TRAIN_FILE, config.IMAGE_SIZE, aug_func, config.BATCH_SIZE)
    ...
    path = config.TRAIN_FILE.split('/')
    weights_path = './weights/' +path[-2] +'/'+ path[-1][:-4]
    pathlib.Path(weights_path).mkdir(parents=True, exist_ok=True)
 
    # Training and Validation of the model
    appr.train_val(loaders=train_loader, criterion=criterion, \
            optimizer=optimizer, num_epochs=config.NUM_EPOCHS, weights_dir=weights_path)

    # Test the model
    #appr.test(loader=test_loader, criterion=criterion, weights_dir=weights_path)
