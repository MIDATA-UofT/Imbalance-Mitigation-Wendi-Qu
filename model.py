...
class ResNet18():
    ...
    def build_model(self):
        # model = resnet18dropout(self.num_classes)
    ...
    def train_val(self, loaders, criterion, optimizer, num_epochs, weights_dir):
        ...
        def score_fn(engine):
            ...
            return -score
        best_model_handler = ModelCheckpoint(dirname=weights_dir,
                                            filename_prefix = "best",
                                            n_saved=1,
                                            score_name="loss",
                                            require_empty=False,
                                            score_function=score_fn)
        train_evaluator.add_event_handler(Events.COMPLETED, best_model_handler, {'model': self.model})
        ...
        # wandb
#        wandb.log({
#            'Best_Epoch_Training_Accuracy': best_metrics['accuracy'], 
#            'Best_Epoch_Training_Loss': best_metrics['celoss'],
#        })
    ...
    #def test(self, loader, criterion, weights_dir):
        ''' Testing forward for most Approaches '''
    #    # Set Up
    #    weights = glob.glob(weights_dir+"/*.pth")[0]
    #    self.build_model()
    ...
    #    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        #    'Test_Precision_Control': metrics['precision'][0],
        #    'Test_Precision_Pneumonia': metrics['precision'][1],
        #    'Test_Recall_Control': metrics['recall'][0],
        #    'Test_Recall_Pneumonia': metrics['recall'][1],
        #    'Test_F1': metrics['f1'],
        #})

        #table = wandb.Table(columns=["CM", "Predicted_Control", "Predicted_Pneumonia"])
        #table.add_data("Actual_Control", metrics['confmatrix'][0][0], metrics['confmatrix'][0][1])
        #table.add_data("Actual_Pneumonia", metrics['confmatrix'][1][0], metrics['confmatrix'][1][1])
    ...
    #    return [
    #        self.trainig_metrics['accuracy'], self.training_metrics['celoss']
